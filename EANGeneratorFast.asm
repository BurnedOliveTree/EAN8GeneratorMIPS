...
.data
	...
	imgdsc:	.word 0
		...
		.word 0
	img:	.space	bytesno
	ifname:	.asciiz "EAN8 - MIPS/templates/512x512.bmp"
	img2:	.space	bytesno
...
# ----------------------------------------------
	...
	paint_code:
		...
		jal	set_mask		# ustawia w $t1 maskę i w $t3 rowsize
	paint_line:
		sb	$t1, 0($t0)		# wpisuje maskę do adresu
		addu	$t0, $t0, $t3		# zwieksza adres o rowsize
		addiu	$a2, $a2, 1
		blt	$a2, $s2, paint_line	# sprawdza czy to nie jest wysokość pliku juz
	paint_check:
		addiu	$t8, $t8, 1
		addiu	$a1, $a1, 1
		blt	$t8, $s7, paint_code
		li	$t8, 0
		addiu	$s3, $s3, 1
...
# ----------------------------------------------
	...
	jump_add_4:
		subu	$t0, $t0, $t6
		sw	$t0, rowsize($s0)
		mul	$s1, $s7, codelen	# szerokość całego kodu EAN8
